...
public class BossController implements Listener {
    ...
    public BossController(Random random, BossManager bossManager, BossBarManager bossBarManager) {
        this.random = random;
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        if (bossManager.getBoss() != null) {
            this.bossBarManager.addBossBar(player);
        } else {
    ...
    public void onBossDamage(EntityDamageEvent event) {
        ...
        double halfHealth = boss.getAttribute(Attribute.GENERIC_MAX_HEALTH).getBaseValue() / 2;

        if (!abilityUsed && boss.getHealth() - event.getFinalDamage() <= halfHealth) {
            abilityUsed = true;
            boss.getWorld().getPlayers().forEach(player -> {
                if (boss.getLocation().distance(player.getLocation()) < 10) {
    ...
    public void onPlayerHitBoss(EntityDamageByEntityEvent event) {
        if (bossManager.getBoss() == null || event.getEntity() != bossManager.getBoss()) return;
        this.bossBarManager.refreshBar();
